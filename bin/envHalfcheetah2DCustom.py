...
class customEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, options: Optional[dict] = None):
        ...
        # Number of actions
        self.ac_len = (self.njoints - 1) # 1 action for each joint except for head (fixed)
    ...
    def reset(self, seed=None):
        ...
        bleg_h = self.body_param[1] * U + self.body_param[5] * U + self.body_param[7] * U + self.body_param[9] * U
        fleg_h = self.body_param[1] * U + self.body_param[11] * U + self.body_param[13] * U + self.body_param[15] * U
        ...
        # Torso sizes
        torsoWidth = self.body_param[0] * U
        torsoHeight = self.body_param[1] * U
        torsoAngle = 0.0
        ...
        # Head sizes
        headWidth = self.body_param[2] * U
        headHeight = self.body_param[3] * U
        headAngle = math.pi / 4.0
        ...
        for i in [-1,+1]:
            # Thigh
            thighWidth = self.body_param[bi] * U
            thighHeight = self.body_param[bi + 1] * U
            # Joint ranges
            thighLowerAngle = self.joint_param[ji]
            thighUpperAngle = self.joint_param[ji + 1]
            thighAngle = self.angle_param[ai]
            ...
            ai += 1
            # Shin
            shinWidth = self.body_param[bi] * U
            shinHeight = self.body_param[bi + 1] * U
            # Joint ranges
            shinLowerAngle = self.joint_param[ji]
            shinUpperAngle = self.joint_param[ji + 1]
            shinAngle = self.angle_param[ai]
            ...
            ai += 1
            # Foot
            footWidth = self.body_param[bi] * U
            footHeight = self.body_param[bi + 1] * U
            # Joint ranges
            footLowerAngle = self.joint_param[ji]
            footUpperAngle = self.joint_param[ji + 1]
            footAngle = self.angle_param[ai]
