...
class Algo(EvoAlgo):
    ...
    def run(self):
        ...
        print("Generalist seed %d Popsize %d %d batchSize %d stepsize %lf noiseStdDev %lf wdecay %d nparams %d" % (self.seed, self.popsize, self.selsize, self.batchSize, self.stepsize, self.noiseStdDev, self.wdecay, nparams))
        ...
        np.save(os.path.join(self.filedir, filename), pop)
        filename = "S%dFitG0.npy" % (seed)
        np.save(os.path.join(self.filedir, filename), self.fmatrix)  
        ...
        for gen in range(self.ngenerations):
            ...
            if (((gen + 1) % (self.saveeach * 10)) == 0):
                ...
                np.save(os.path.join(self.filedir, filename), popm)
                filename = "S%dG%dv.npy" % (seed, gen + 1)
                np.save(os.path.join(self.filedir, filename), popv)
    ...
    def test(self, testparam):
        ...
        if (not parsen[0] in ["P", "p", "m", "M","c", "C", "K"]):
            self.testusage()
        ...
        #  also save the best individual of the last generation
        if (parsen[0] == "m"):
            popfile = "S%dG%d.npy" % (seed,int(parsen[1]))
            pop = np.load(popfile)
            popshape = pop.shape
            ...
            for pp in range(2):
                ...
                cgen = int(parsen[1])
                performanceid = np.zeros(popsize)
                while (cgen >= 0):
                    ...
                    for i1 in range(popsize):
                        ...
                        for i2 in range(popsize):
                            ...
                            ind_rew += rew
                            if (pp == 0):
                                performanceid[i1] += rew
                            else:
                                performanceid[i1] += 1.0 - rew
                    ...
                    cgen -= int(parsen[2])
                print("")
                bestid = np.argmax(performanceid)
                if (pp == 0):
                    bestgeno = pop[bestid]
                    bestfile = "S%dBestPop1.npy" % (seed)
                else:
                    bestgeno = pop[popsize+bestid]
                    bestfile = "S%dBestPop2.npy" % (seed)
        ...
        # "M-n1-n2, Master tournament, test pop of all generations against every generations up to generation n1, every n2 generations
        #  save the data in a master.npy file 
        if (parsen[0] == "M"):
            ...
            bestrew1 = ""
            bestrew2 = ""
            lastgen = int(parsen[1])
            everygen = int(parsen[2])
            ntests = int(lastgen / everygen) + 1
            master = np.zeros((ntests, ntests), dtype=np.float64)
            print("seed %d: master tournament, postevaluate all generations up to %d against all generations, every %d generations" % (seed, lastgen, everygen))
            g1 = 0
            g2 = 0
            for p1 in range(ntests):
                g1 = int(p1 * everygen)
                pop1file = "S%dG%d.npy" % (seed,g1)
                pop1 = np.load(pop1file)
                for p2 in range(ntests):
                    g2 = int(p2 * everygen)
                    pop2file = "S%dG%d.npy" % (seed,g2)
                    pop2 = np.load(pop2file)
                    tot_rew = 0
                    for i1 in range(popsize):
                        ind_rew = 0
                        for i2 in range(popsize):
                            self.policy.set_trainable_flat(np.concatenate((pop1[i1], pop2[popsize+i2])))
                            rew, eval_length = self.policy.rollout(1)
                            tot_rew += rew
                    print("%.2f " % (tot_rew / (popsize*popsize)), end = '', flush=True)
                    master[p1, p2] = tot_rew / (popsize * popsize)
                print("")
            masterfile = "masterS%d.npy" % (seed)
            np.save(masterfile, master)
