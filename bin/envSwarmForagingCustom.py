...
# Constants
MAX_VEL = 0.25
MAX_ROT = math.pi / 20.0 # This should be quite realistic
MAX_DIFF = 2.0 * MAX_VEL
# Field of view (angle)
FOV = math.pi / 2.0#(2.0 * math.pi) / 3.0#2.0 * math.pi# / 2.0
# Number of sectors
NUM_SECTORS = 6
...
class customEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None, options: Optional[dict] = None):
        ...
        self.neaten = 0
        # No noise in the default setting
        self.noisy = False
        self.noise = NOISE
        if options is not None:
            ...
            try:
                ...
                pass
            # Get the noisy flag (to implement a more realistic scenario)
            try:
                opt = int(options['noisy'])
                self.noisy = bool(opt)
            except:
                pass
            # Get the noisy flag (to implement a more realistic scenario)
        ...
        self.ob_len = (NUM_SECTORS * 3) + NUM_IR # For each sector the proportion of red/green/blue elements detected + number of IR sensors
        # Number of actions
    ...
    def reset(self, seed=None):
        ...
        for i in range(self.nobjects):
            ...
            # Set the color to either red or green
            if i % 2 == 0:
                # Poison
                obj.color1 = (255,0,0)
                obj.color2 = (255,0,0)
        ...
        # Reset the number of eaten foods
        self.neaten = 0
    ...
    def step(self, action): # _step
        ...
        for i in range(self.nagents):
            act = action[(i * self.ac_len):((i * self.ac_len) + self.ac_len)]
            # Agent position
            ...
            if px <= START_W + AGENT_RADIUS or px >= START_W + ARENA_WIDTH/SCALE - AGENT_RADIUS or py <= START_H + AGENT_RADIUS or py >= START_H + ARENA_HEIGHT/SCALE - AGENT_RADIUS:
                ...
                px, py = self.agents[i].position
                angle = prevAngle#self.np_random.uniform(-math.pi, math.pi)
            ...
            for j in range(self.nagents):
                if i != j:
                    ...
                    if dist < 1e-6: # TO BE FURTHER FIXED
                        # Restore previous position
                        px, py = self.agents[i].position
                        angle = prevAngle#self.np_random.uniform(-math.pi, math.pi)
        ...
        for i in range(self.nagents):
                ...
                for s in robs:
                    if s not in sectors:
                        ...
                        if dist < cdist:
                            dists[idx] = dist
                            colors[s] = (0, 0, 255)
            for objs, dist, col in zip(objsectors, objdists, objcolors):
                for s in objs:
                    if s not in sectors:
                        sectors.append(s)
                        dists.append(dist)
            ...
            for s, d in zip(sectors, dists):
                if colors[s] == (255, 0, 0):
                    ...
                    if self.noisy:
                        # Add uniform noise
                        objlist[s] += self.np_random.uniform(-self.noise, self.noise)
                        # Check sensory readings (to be bounded in the range [0,1])
                        if objlist[s] < 0.0:
                            objlist[s] = 0.0
                        if objlist[s] > 1.0:
                            objlist[s] = 1.0
                    ...
                    if self.noisy:
                        # Add uniform noise
                        objlist[NUM_SECTORS + s] += self.np_random.uniform(-self.noise, self.noise)
                        # Check sensory readings (to be bounded in the range [0,1])
                        if objlist[NUM_SECTORS + s] < 0.0:
                            objlist[NUM_SECTORS + s] = 0.0
                        if objlist[NUM_SECTORS + s] > 1.0:
                            objlist[NUM_SECTORS + s] = 1.0
                else:
                    objlist[2 * NUM_SECTORS + s] = (1.0 - d / MAX_DIST)
                    if self.noisy:
                        # Add uniform noise
                        objlist[2 * NUM_SECTORS + s] += self.np_random.uniform(-self.noise, self.noise)
                        # Check sensory readings (to be bounded in the range [0,1])
                        if objlist[2 * NUM_SECTORS + s] < 0.0:
                            objlist[2 * NUM_SECTORS + s] = 0.0
                        if objlist[2 * NUM_SECTORS + s] > 1.0:
                            objlist[2 * NUM_SECTORS + s] = 1.0
            # Evaluate the addition of infrared sensors for walls
            for ir in range(NUM_IR):
                proj_x = px + IR_RANGE * math.cos(ir * math.pi / 4.0)
                proj_y = py + IR_RANGE * math.sin(ir * math.pi / 4.0)
                if proj_x <= START_W: 
                    # Left wall detected
                    objlist[3 * NUM_SECTORS + ir] = 1.0 - ((px - START_W) / IR_RANGE)
                if proj_x >= START_W + ARENA_WIDTH/SCALE:
                    # Right wall detected
                    objlist[3 * NUM_SECTORS + ir] = 1.0 - ((START_W + ARENA_WIDTH/SCALE - px) / IR_RANGE)
                if proj_y <= START_H:
                    # Bottom wall detected
                    objlist[3 * NUM_SECTORS + ir] = 1.0 - ((py - START_H) / IR_RANGE)
                if proj_y >= START_H + ARENA_HEIGHT/SCALE:
                    # Top wall detected
                    objlist[3 * NUM_SECTORS + ir] = 1.0 - ((START_H + ARENA_HEIGHT/SCALE - py) / IR_RANGE)
            obs += objlist
        return obs
