...
class BulletPolicy(Policy):
    ...
    def rollout(self, ntrials, render=False, seed=None):   # evaluate the policy for one or more episodes 
        ...
        for trial in range(ntrials):
            self.ob, _ = self.env.reset(seed=None)   # reset the environment
            ...
            while t < self.maxsteps:
                ...
                self.ob, r, terminated, truncated, _ = self.env.step(self.ac)  # perform a simulation step
                rew += r
                t += 1
                if (self.test > 0):
                    if (self.test == 1):
    ...
    def rollout(self, ntrials, render=False, seed=None):
        ...
        for trial in range(ntrials):
            ...
            while t < self.maxsteps:
                ...
                self.ob, r, terminated, truncated, _ = self.env.step(action) # perform a simulation step
                rew += r
                t += 1
                if render:
                    if (self.test == 1):
