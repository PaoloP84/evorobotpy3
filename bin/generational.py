...
class Algo(EvoAlgo):
    ...
    def computePairs(self, size):
        assert size % 2 == 0, "Cannot create pairs with an odd number of elements!!!"
        npairs = int(size / 2)
        elems = np.arange(size, dtype=np.int64)
        pairs = -1 * np.ones((npairs, 2), dtype=np.int64)
        cnt = 0
        while len(elems) > 0:
            # Extract the first element of the pair
            idx = np.random.randint(0, len(elems))
            pairs[cnt][0] = elems[idx]
            # Delete the corresponding entry in elems
            elems = np.delete(elems, idx)
            # Extract the second element of the pair
            idx = np.random.randint(0, len(elems))
            pairs[cnt][1] = elems[idx]
            # Delete the corresponding entry in elems
            elems = np.delete(elems, idx)
            # Move to next pair
            cnt += 1
        assert cnt == npairs, "Something went wrong!!!"
        return pairs
    ...
    def run(self):
        ...
        while ceval < self.maxsteps:
            ...
            # If task is multi-agent and requires heterogeneity, we need to compute pairs of individuals to be tested
            heterogeneous = self.policy.is_heterogeneous
            if heterogeneous > 0:
                # Compute pairs of individuals to be tested
                pairs = self.computePairs(self.popSize)
            ...
            #for i in range(self.popSize):
                if heterogeneous == 0:
                    ...
                    ids = pairs[int(i / 2)]
                    self.policy.set_trainable_flat(np.concatenate((self.pop[ids[0]], self.pop[ids[1]])))
                    neval = 2
                eval_rews, eval_length = self.policy.rollout(self.policy.ntrials)  # evaluate the individual
                if heterogeneous == 0:
                    fitness[i] = eval_rews                        # store fitness
                else:
                    fitness[ids[0]] = eval_rews
                    fitness[ids[1]] = eval_rews
                ...
                if heterogeneous == 0:
                    ...
                    self.updateBest(fitness[ids[0]], self.pop[ids[0]])           # Update data if the current offspring is better than current best
                    self.updateBest(fitness[ids[1]], self.pop[ids[1]])           # Update data if the current offspring is better than current best
                i += neval
