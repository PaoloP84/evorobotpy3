"""
   ...
   Pagliuca, P. (2026). "Leveraging High Morphology Rates in Body-Brain Co-Evolution: A Case Study on 2D Robot Locomotion".
...
HULL_POLY = [(-30, +9), (+6, +9), (+34, +1), (+34, -8), (-30, -8)] # Coordinates of the hull
LEG_DOWN = -8 / SCALE
LEG_W, LEG_H = 8 / SCALE, 34 / SCALE # Width/height of a leg
...
class customEnv(BipedalWalker, EzPickle):
    """
    ## Description
    ...
    def __init__(self, render_mode: Optional[str] = None, options: Optional[dict] = None):
        # We need to read the option before calling parent
        self.rate = MOD_RATE
        self.hardcore = False
        if options is not None:
            ...
            try:
                ...
                pass
        BipedalWalker.__init__(self, render_mode, self.hardcore)
        self.isopen = True
        ...
        self.nsegs = 4 # Width and height (2 upper legs + 2 lower legs)
        self.ndensities = 3 #0 # Hull density + upper leg density + lower leg density
        self.nangs = 4 # Angular ranges ( 2 upper legs + 2 lower legs)
        self.ncoeffs = 1 # Coefficient to rescale hull vertices
        self.nparams = (self.nsegs + self.ndensities + self.nangs + self.ncoeffs) # 1 coefficient for hull vertices + 8 pair width, heights (2 * 2 * 2 legs) + 8 angular ranges (2 * 2 * 2 legs) + 5 density values (hull + 2 * 2 legs)
        self.params = np.zeros(self.nparams, dtype=np.float64)
        self.factors = np.ones(self.nparams, dtype=np.float64)
    ...
    def computeFactor(self, idx):
        ...
        factor = 0.0
        if idx >= 0 and idx < self.nsegs:
            # Segment sizes
            # Factors are computed as tanh(p), where p is the parameter
            factor = (1.0 + np.tanh(self.params[idx]) * self.rate)
        elif idx >= self.nsegs and idx < self.nsegs + self.ndensities:
            # Densities
            # Factors are computed as tanh(p), where p is the parameter
            factor = (1.0 + np.tanh(self.params[idx]) * self.rate)
        elif idx >= self.nsegs + self.ndensities and idx < self.nsegs + self.ndensities + self.nangs:
            # Joint ranges
            # Angle factors must belong to range [-1,1]. Values outside boundaries are cut
            param = np.tanh(self.params[idx]) * self.rate # Very small variation to avoid unexpected morphologies/behaviors
            factor = (1.0 + param / 2.0)
        else:
            # Coefficient to resize the hull
            factor = 2.0 ** np.tanh(self.params[idx]) # tanh = -1 -> coeff = 0.5; tanh = 1 -> coeff = 2
        return factor
    ...
    def reset(
        ...
        *,
        ...
        # Default values for original bipedal walker
        HULL_POLY = [(-30, +9), (+6, +9), (+34, +1), (+34, -8), (-30, -8)] # Coordinates of the hull
        LEG_DOWN = -8 / SCALE
        LEG_W, LEG_H = 8 / SCALE, 34 / SCALE # Width/height of a leg
        
        # Here we compute the morphological parameters for the robot
        # Coefficient for the hull
        idx = self.nparams - 1
        coeff = self.computeFactor(idx)
        # Update hull poly
        HULL_POLY_MOD = []
        for elem in HULL_POLY:
            x, y = elem
            HULL_POLY_MOD.append((coeff * x, coeff * y))
        last_x, last_y = HULL_POLY_MOD[-1]
        leg_down = last_y / SCALE
        # Update density
        idx = self.nsegs
        coeff_density = None
        coeff_d = self.computeFactor(idx)
        """
        if abs(coeff_d - coeff) <= 0.2:
            coeff_density = coeff_d
        else:
            coeff_density = coeff
        """
        hull_density = coeff_d#ensity#coeff * 5.0#self.computeFactor(5.0, idx)
        # Modified hull fixture definition
        HULL_FD = fixtureDef(
            shape=polygonShape(vertices=[(x / SCALE, y / SCALE) for x, y in HULL_POLY_MOD]),
        ...
        # Width and height of the legs
        # High leg
        idx = 0
        leg_width_coeff = self.computeFactor(idx)
        leg_width = LEG_W * leg_width_coeff
        idx += 1
        leg_height_coeff = self.computeFactor(idx)
        leg_height = LEG_H * leg_height_coeff
        # Low leg
        idx += 1
        low_leg_width_coeff = self.computeFactor(idx)
        low_leg_width = 0.8 * LEG_W * low_leg_width_coeff
        idx += 1
        low_leg_height_coeff = self.computeFactor(idx)
        low_leg_height = LEG_H * low_leg_height_coeff
        # Density
        # High leg
        idx = self.nsegs + 1
        coeff_density = None
        coeff_d = self.computeFactor(idx)
        """
        if abs(coeff_d - (leg_width_coeff * leg_height_coeff)) <= 0.2:
            coeff_density = coeff_d
        else:
            coeff_density = (leg_width_coeff * leg_height_coeff)
        """
        leg_density = coeff_d#ensity#leg_width_coeff * leg_height_coeff#self.computeFactor(1.0, idx)
        # Low leg
        idx += 1
        coeff_density = None
        coeff_d = self.computeFactor(idx)
        """
        if abs(coeff_d - (low_leg_width_coeff * low_leg_height_coeff)) <= 0.2:
            coeff_density = coeff_d
        else:
            coeff_density = (low_leg_width_coeff * low_leg_height_coeff)
        """
        low_leg_density = coeff_d#ensity#low_leg_width_coeff * low_leg_height_coeff#self.computeFactor(1.0, idx)
        # Modified leg fixture definition
        # High leg
        LEG_FD = fixtureDef(
            shape=polygonShape(box=(leg_width / 2, leg_height / 2)),
            density=leg_density,
            restitution=0.0,
            categoryBits=0x0020,
            maskBits=0x001,
        ...
        # Low leg
        LOWER_FD = fixtureDef(
            shape=polygonShape(box=(low_leg_width / 2, low_leg_height / 2)),
        ...
        # Joint angles
        # High leg
        idx = self.nsegs + self.ndensities
        leg_min_ang = -0.8 * self.computeFactor(idx)
        idx += 1
        leg_max_ang = 1.1 * self.computeFactor(idx)
        # Low leg
        idx += 1
        low_leg_min_ang = -1.6 * self.computeFactor(idx)
        idx += 1
        low_leg_max_ang = -0.1 * self.computeFactor(idx)

        init_x = TERRAIN_STEP * TERRAIN_STARTPAD / 2
        init_y = TERRAIN_HEIGHT + leg_height + low_leg_height
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(init_x, init_y - leg_height / 2 - leg_down),#LEG_H / 2 - LEG_DOWN),
                angle=(i * 0.05),
                fixtures=LEG_FD,
            )
            ...
            rjd = revoluteJointDef(
                bodyA=self.hull,
                bodyB=leg,
                localAnchorA=(0, leg_down),#LEG_DOWN),
                localAnchorB=(0, leg_height / 2),#LEG_H / 2),
                enableMotor=True,
                enableLimit=True,
                maxMotorTorque=MOTORS_TORQUE,
                motorSpeed=i,
                lowerAngle=leg_min_ang,#-0.8,
                upperAngle=leg_max_ang,#1.1,
            )
            self.legs.append(leg)
            self.joints.append(self.world.CreateJoint(rjd))

            lower = self.world.CreateDynamicBody(
                position=(init_x, init_y - leg_height - low_leg_height / 2 - leg_down),#LEG_H * 3 / 2 - LEG_DOWN),
                angle=(i * 0.05),
                fixtures=LOWER_FD,
            )
            ...
            rjd = revoluteJointDef(
                bodyA=leg,
                bodyB=lower,
                localAnchorA=(0, -leg_height / 2),#-LEG_H / 2),
                localAnchorB=(0, low_leg_height / 2),#LEG_H / 2),
                enableMotor=True,
                enableLimit=True,
                maxMotorTorque=MOTORS_TORQUE,
                motorSpeed=1,
                lowerAngle=low_leg_min_ang,#-1.6,
                upperAngle=low_leg_max_ang,#-0.1,
            )
